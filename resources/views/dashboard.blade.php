            ...
            <table class="table">
                ...
                <tbody class="table-tbody">
                    ...
                    <tr class="group"
                        ...
                        data-title="{{ $application->job_title }}"
                        data-status="{{ $application->status }}"
                        data-source="{{ $application->source }}"
                        data-url="{{ $application->job_url }}"
                        data-description="{{ $application->job_description }}"
                        data-cover-letter="{{ $application->cover_letter_text }}"
                        data-email-to="{{ $application->email_to }}"
                        data-email-body="{{ $application->email_body }}"
                        data-notes="{{ $application->notes }}"
                    >
                        <td class="table-td">
                            <div class="flex items-center gap-3">
                                <div class="flex size-9 items-center justify-center rounded-xl bg-primary/5 text-primary font-bold border border-primary/10 text-sm">
                        ...
                        <td class="table-td text-muted-foreground">{{ $application->applied_at?->format('M d, Y') ?? '—' }}</td>
                        <td class="table-td text-right relative">
                            <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="/applications/{{ $application->id }}" class="btn-ghost text-xs px-3 py-1">View</a>
                                <button onclick="openEditModal(this.closest('tr'))" class="btn-ghost text-xs px-3 py-1">Edit</button>
                                <form action="/applications/{{ $application->id }}" method="POST" onsubmit="return confirm('Delete this application?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn-ghost text-xs px-3 py-1 text-destructive">Delete</button>
                                </form>
                            </div>
                        </td>
                    </tr>
